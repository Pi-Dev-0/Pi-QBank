...
class HSCBusinessOrgSecondPaper extends StatefulWidget {
  const HSCBusinessOrgSecondPaper({super.key});

  @override
  State<HSCBusinessOrgSecondPaper> createState() => _HSCBusinessOrgSecondPaperState();
}

class _HSCBusinessOrgSecondPaperState extends State<HSCBusinessOrgSecondPaper> {
  ...
  Future<void> fetchQuestionPapers() async {
    ...
    const String cacheKey = 'hsc_business_org_second';
        ...
        () async {
          ...
          if (response.statusCode == 200) {
            ...
            return (data['papers'] as List)
                .where((paper) => 
                    paper['subject'].toString().toLowerCase() == 'business_org_second')
                .map((paper) => {
                      'title': paper['title'],
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const CustomAppBar(title: 'ব্যবসায় সংগঠন ২য় পত্র - HSC'),
