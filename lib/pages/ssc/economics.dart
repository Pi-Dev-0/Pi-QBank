...
class SSCEconomics extends StatefulWidget {
  const SSCEconomics({super.key});

  @override
  State<SSCEconomics> createState() => _SSCEconomicsState();
}

class _SSCEconomicsState extends State<SSCEconomics> {
  ...
  Future<void> fetchQuestionPapers() async {
    ...
    const String cacheKey = 'ssc_economics';
        ...
        () async {
          ...
          if (response.statusCode == 200) {
            ...
            final filteredPapers = (data['papers'] as List)
                .where((paper) => paper['subject'].toString().toLowerCase() == 'economics')
                .map((paper) => {
                      'title': paper['title'],
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const CustomAppBar(title: 'অর্থনীতি - SSC'),
