...
class SSCFinance extends StatefulWidget {
  const SSCFinance({super.key});

  @override
  State<SSCFinance> createState() => _SSCFinanceState();
}

class _SSCFinanceState extends State<SSCFinance> {
  ...
  Future<void> fetchQuestionPapers() async {
    ...
    const String cacheKey = 'ssc_finance';
        ...
        () async {
          ...
          if (response.statusCode == 200) {
            ...
            final filteredPapers = (data['papers'] as List)
                .where((paper) =>
                    paper['subject'].toString().toLowerCase() == 'finance')
                .map((paper) => {
                      'title': paper['title'],
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const CustomAppBar(title: 'ফাইন্যান্স - SSC'),
