...
class SSCBiology extends StatefulWidget {
  const SSCBiology({super.key});

  @override
  State<SSCBiology> createState() => _SSCBiologyState();
}

class _SSCBiologyState extends State<SSCBiology> {
  ...
  Future<void> fetchQuestionPapers() async {
    ...
    const String cacheKey = 'ssc_biology';
        ...
        () async {
          ...
          if (response.statusCode == 200) {
            ...
            if (data != null && data is List) {
              List<Map<String, dynamic>> filteredPapers = data
                  .map((item) => Map<String, dynamic>.from(item))
                  .where((paper) =>
                      paper['subject'].toString().toLowerCase() == 'biology')
                  .toList();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const CustomAppBar(title: 'জীববিজ্ঞান - SSC'),
