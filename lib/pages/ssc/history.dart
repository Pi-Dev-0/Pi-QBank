...
class SSCHistory extends StatefulWidget {
  const SSCHistory({super.key});

  @override
  State<SSCHistory> createState() => _SSCHistoryState();
}

class _SSCHistoryState extends State<SSCHistory> {
  ...
  Future<void> fetchQuestionPapers() async {
    ...
    const String cacheKey = 'ssc_history';
        ...
        () async {
          ...
          if (response.statusCode == 200) {
            ...
            final filteredPapers = (data['papers'] as List)
                .where((paper) => 
                    paper['subject'].toString().toLowerCase() == 'history')
                .map((paper) => {
                      'title': paper['title'],
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const CustomAppBar(title: 'ইতিহাস - SSC'),
