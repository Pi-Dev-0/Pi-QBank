...
class SSCChemistry extends StatefulWidget {
  const SSCChemistry({super.key});

  @override
  State<SSCChemistry> createState() => _SSCChemistryState();
}

class _SSCChemistryState extends State<SSCChemistry> {
  ...
  Future<void> fetchQuestionPapers() async {
    ...
    const String cacheKey = 'ssc_chemistry';
        ...
        () async {
          ...
          if (response.statusCode == 200) {
            ...
            return (data['papers'] as List)
                .where((paper) =>
                    paper['subject'].toString().toLowerCase() == 'chemistry')
                .map((paper) => {
                      'title': paper['title'],
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const CustomAppBar(title: 'রসায়ন - SSC'),
