...
class SSCBusiness extends StatefulWidget {
  const SSCBusiness({super.key});

  @override
  State<SSCBusiness> createState() => _SSCBusinessState();
}

class _SSCBusinessState extends State<SSCBusiness> {
  ...
  Future<void> fetchQuestionPapers() async {
    ...
    const String cacheKey = 'ssc_business';
        ...
        () async {
          ...
          if (response.statusCode == 200) {
            ...
            final filteredPapers = (data['papers'] as List)
                .where((paper) => paper['subject'].toString().toLowerCase() == 'business')
                .map((paper) => {
                      'title': paper['title'],
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const CustomAppBar(title: 'ব্যবসায় উদ্যোগ - SSC'),
