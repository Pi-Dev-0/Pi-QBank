...
class SSCAccounting extends StatefulWidget {
  const SSCAccounting({super.key});

  @override
  State<SSCAccounting> createState() => _SSCAccountingState();
}

class _SSCAccountingState extends State<SSCAccounting> {
  ...
  Future<void> fetchQuestionPapers() async {
    ...
    const String cacheKey = 'ssc_accounting';
        ...
        () async {
          ...
          if (response.statusCode == 200) {
            ...
            final filteredPapers = (data['papers'] as List)
                .where((paper) =>
                    paper['subject'].toString().toLowerCase() == 'accounting')
                .map((paper) => {
                      'title': paper['title'],
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const CustomAppBar(title: 'হিসাববিজ্ঞান - SSC'),
